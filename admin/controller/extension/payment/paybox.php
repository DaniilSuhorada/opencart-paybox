...
class ControllerExtensionPaymentPaybox extends Controller
{
    public function index()
    {
        ...
        $data['api_url_list'] = explode(',', 'api.paybox.money,api.paybox.ru');

        if (isset($this->request->post['payment_paybox_api_url'])) {
            $data['payment_paybox_api_url'] = $this->request->post['payment_paybox_api_url'];
        } else {
            $data['payment_paybox_api_url'] = $this->config->get('payment_paybox_api_url');
        }
        ...
        if (isset($this->request->post['payment_paybox_status'])) {
            $data['payment_paybox_status'] = $this->request->post['payment_paybox_status'];
        } else {
    ...
    {
        if (!$this->user->hasPermission('modify', 'extension/payment/paybox')) {
